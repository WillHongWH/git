...
* 若 $$X$$ 呈現二項分配，則其期望值與變異數分別為：  $$\mu_X=X_1P_1+X_2P_2+...+X_nP_n=np$$   $$\sigma^2_X=(X_1-\mu_X)^2P_1+(X_2-\mu_X)^2P_2+...+(X_N-\mu_X)^2P_n=np(1-p)$$ 
* 若 $$\hat{p}=X/n$$ ，則 $$\hat{p}$$ 的期望值與變異數分別為：   $$\mu _\hat{P}=(X_1/n)p_1+(X_2/n)p_2+...+(X_n/n)p_n=(1/n)np=p$$   $$\sigma^2_\hat{p}((X_1/n)-\mu_\hat{p})^2p_1+...+((X_N/n)-\mu_\hat{p})^2 \\ =(1/n^2) \times np(1-p)= p(1-p)/n$$ 
* 換言之， $$\hat{p}$$ 的標準差為 $$\sigma_\hat{p}=\sqrt{\frac{p(1-p)}{n}}$$ ˆpˆ\(1\)Pppn  
